...
export const removeRole = async (req, res, user_id, roleKey) => {
	try {
		...
		if (!validUser.rows.length > 0) {
			return res.status(405).json({
				statusCode: 405,
				message: `У пользователя отсутствует роль ${role.rows[0].description}`,
			})
		}
		...
		return res.status(200).json({
			statusCode: 200,
			message: `Роль ${role.rows[0].description} у пользователя ${user.rows[0].name} ${user.rows[0].surname} успешно удалена!`,
			insertRole,
		})
	} catch (e) {
		console.log(e)
